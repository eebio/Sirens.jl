...
```@example dupcomp
using OrdinaryDiffEq, Agents, Random, Sirens, CairoMakie
...
!!! note "Sorting of Connectors"
    You may wonder about the order that connectors are applied. If we adjusted the IDs after applying `conn2`, we would be using an old value for IDs. Generally, we utilise the order specified in the SirenProblem. For this reason, the `#ids` from `conn3` is applied first, as will be specified in the order of the connectors vector.
